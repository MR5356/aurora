...
type Service struct {
	userDB     database2.Mapper[*User]
	groupDB    *database2.BaseMapper[*Group]
	relationDB *database2.BaseMapper[*Relation]
}

func GetService() *Service {
	once.Do(func() {
		service = &Service{
			userDB:     database2.NewCachedMapper(database2.GetDB(), &User{}, cache.GetCache()),
			groupDB:    database2.NewMapper(database2.GetDB(), &Group{}),
			relationDB: database2.NewMapper(database2.GetDB(), &Relation{}),
		}
	})
	return service
}
...
func (s *Service) SetUserStatus(user *User, status int) error {
	return s.userDB.GetDB().Model(&User{}).Where(user).Update("status", status).Error
}
...
func (s *Service) ResetPassword(user *ResetPasswordRequest, IsAdmin bool) error {
	...
	if u, err := s.userDB.Detail(&User{ID: user.Username}); err != nil {
		...
		if !IsAdmin && u.Password != user.Old {
			...
			return s.userDB.GetDB().Model(&User{}).Where("id = ?", u.ID).Update("password", user.New).Error
		}
	}
}
...
func (s *Service) Initialize() error {
	if err := database2.GetDB().AutoMigrate(&User{}, &Group{}, &Relation{}); err != nil {
		return err
	}
	...
	if err := s.userDB.GetDB().FirstOrCreate(adminUser).Error; err != nil {
		return err
	}
