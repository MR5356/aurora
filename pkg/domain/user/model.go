...
func (u *User) IsBanned() bool {
	if r, err := database.NewMapper(database.GetDB(), &User{}).Detail(&User{ID: u.ID}); err != nil {
		return true
	} else {
...
func (u *User) IsAdmin() bool {
	if r, err := database.NewMapper(database.GetDB(), &Relation{}).Detail(&Relation{UserID: u.ID}); err != nil {
		return false
	} else {
